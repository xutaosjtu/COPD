...
plot(S4pls$scores[,1:3],col=color[c(4,8)][interaction(COPD.data$exercise[which(COPD.data$exercise == 1)],COPD.data$COPD[which(COPD.data$exercise == 1)])], pch=c(17, 19)[COPD.data$SEX[which(COPD.data$exercise == 1)]], main = "PLS plot of COPD vs non COPD")
#plot(S4pls$scores[,1:2],col = color[data$COPD[which(COPD.data$exercise == 1)]], pch=c(17, 19)[COPD.data$SEX[which(COPD.data$exercise == 1)]])
